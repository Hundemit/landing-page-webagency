...
const FeaturesPage2 = ({ className }: { className?: string }) => {
  ...
  return (
    <>
      ...
      {/* <div ref={targetRef} className={cn("w-full container duration-300 mx-auto flex-col min-h-fit justify-center h-[100vh]", className)}>
        <div ref={cardRef} style={{ transform: "translateY(50%)" }} className="sticky top-48 mx-auto p-10 rounded-xl will-change-transform">
          <h2 className="text-5xl font-bold text-pretty text-primary-foreground">Unsere Leistungen.</h2>
          <div className="mt-10 sm:mt-16 grid lg:grid-cols-3 gap-8 mx-auto">
            {serviceData.map((feature) => (
              <div key={feature.title} className="flex flex-col gap-6 rounded-xl hover:bg-primary-foreground/20 hover:scale-105 transition-all duration-300 p-6">
                <feature.icon className="size-8 text-primary-foreground" />
                <span className="font-bold text-primary-foreground text-2xl">{feature.title}</span>
                <p className="mt-1 text-primary-foreground/80 text-base"> {feature.description} </p>
                <p className="text-primary-foreground text-base font-bold"> Mehr Infos </p>
              </div>
            ))}
          </div>
        </div>
