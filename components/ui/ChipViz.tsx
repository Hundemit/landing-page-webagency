...
const ChipViz = () => {
  const createVariants = ({ scale, delay }: { scale: number; delay: number }): Variants => ({
    initial: { scale: 1 },
    animate: {
      scale: [1, scale, 1],
      transition: {
        ...
        <motion.div
          ...
          <motion.div
            ...
            <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-full bg-black/40 shadow-xs shadow-white/40 will-change-transform">
              ...
              />
              <motion.div variants={createVariants({ scale: 1.02, delay: 0.4 })} initial="initial" animate="animate" className="absolute inset-[6px] rounded-full bg-white/10 p-1 backdrop-blur-[1px]">
