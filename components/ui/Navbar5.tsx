  ...
  return (
    <header
      className={cn(
        ...
        open === true ? "h-52" : "h-16",
        scrolled || open === true ? "backdrop-blur-nav max-w-3xl border-gray-100 bg-white/80 shadow-xl shadow-black/5 dark:border-white/15 dark:bg-black/70" : "bg-white/0 dark:bg-gray-950/0"
      )}>
      <div className="w-full md:my-auto">
        <div className="relative flex items-center justify-between">
          ...
          <Button className="hidden h-10 font-semibold md:flex">Book a demo</Button>
          <div className="flex gap-x-2 md:hidden">
            <Button>Book demo</Button>
            <Button onClick={() => setOpen(!open)} variant="outline" className="aspect-square p-2">
              {open ? <RiCloseLine aria-hidden="true" className="size-5" /> : <RiMenuLine aria-hidden="true" className="size-5" />}
            </Button>
          </div>
        </div>
        <nav className={cn("my-6 flex text-lg ease-in-out will-change-transform md:hidden", open ? "" : "hidden")}>
